    ...
    if($page == "home"){
        ...
        echo "<div class='recept_list'>";
            foreach($recept_rows as $recept){
                echo "<div class='recept_button' onclick=\"location.href ='?page=$recept[1]'\"><img src='PICS/$recept[5]' alt='$recept[1]' class='recept_pic'>$recept[1]</div>";
            }
    }
    ...
    if($page == "filtr"){
        ...
        if(!empty($_POST)){
            ...
            foreach($data as $rows){
                echo "<div class='recept_button' onclick=\"location.href ='?page=$rows[NÁZEV]'\"><img src='PICS/$rows[OBRAZEK]' alt='$rows[NÁZEV]'class='recept_pic'>$rows[NÁZEV]</div>";
            };
            echo "</div>";
            }
            else{ 
...
// Zobrazení seznamu receptů v dané kategorii
    if(is_numeric($page)){
        ...
        foreach($query as $recept_data){
            echo "<div class='recept_button' onclick=\"location.href ='?page=$recept_data[nazev]'\"><img src='PICS/$recept_data[obrazek]' alt='$recept_data[nazev]' class='recept_pic'>$recept_data[nazev]</div>";
        }
    }
